...
void test(char *spt, char *chars, int expected) {
    strcpy(search_pattern.current, spt);
    int res = mark_pattern(chars, 0, strlen(chars), strlen(chars));
    
    if (res != expected)
        printf("%2d %2d (searched \"%s\" in \"%s\")\n", expected, res, spt, chars);
}
